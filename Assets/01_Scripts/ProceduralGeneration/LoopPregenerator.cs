...
namespace HuntingGame.ProceduralGeneration
{
	public class LoopPregenerator : MonoBehaviour
	{
		...
		{
			bool overlaps = DungeonGeometryUtils.IsOverlappingAny(room, currentLoop, targetSocket.room);
			if (overlaps)
				Debug.Log($"{room.roomName} is hitting an existing room in the loop");
			return overlaps;
		}
		...
		private string GetCombinedFootprintSignature()
		{
			...
			{
				Vector2[] roomVertices = DungeonGeometryUtils.GetWorldFootprint(room);
